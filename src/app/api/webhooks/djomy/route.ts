...
export async function POST(request: NextRequest) {
  ...
  if (!signatureHeader) {
    logger.warn('[Djomy Webhook] Missing X-Webhook-Signature header')
  ...
  if (!isValid) {
    logger.error('[Djomy Webhook] Invalid signature:', signatureHeader.substring(0, 20))
  ...
  try {
    ...
    logger.error('[Djomy Webhook] Invalid JSON payload')
  ...
  logger.log('[Djomy Webhook] Event:', { eventType, eventId: eventId.slice(0, 12), transactionId: transactionId.slice(0, 8), reference: reference.slice(0, 12) })
  ...
  if (!isSuccessEvent && !isFailureEvent) {
    logger.log('[Djomy Webhook] Ignored event type:', eventType)
    return NextResponse.json({ received: true })
  }

  // ── 5. Filtre : uniquement les références abonnement (SUB_) ─────────────
  if (!reference.startsWith('SUB_')) {
    logger.log('[Djomy Webhook] Non-subscription reference, ignored:', reference.slice(0, 12))
    return NextResponse.json({ received: true })
  }

  if (!transactionId) {
    logger.error('[Djomy Webhook] Missing transactionId')
  ...
  if (eventId) {
    ...
    if (existingEvent) {
      logger.log('[Djomy Webhook] Duplicate eventId, already processed:', eventId)
    ...
    await supabase.from('webhook_events').insert({
      ...
      if (error) logger.warn('[Djomy Webhook] webhook_events insert error:', error.message)
    })
  }
  ...
  try {
    ...
    logger.log('[Djomy Webhook] Server verification:', { status: verified.status, isSuccess })
    ...
    if (subError || !sub) {
      logger.error('[Djomy Webhook] Subscription not found for reference:', reference.slice(0, 12))
      return NextResponse.json({ error: 'Subscription not found' }, { status: 404 })
    }

    // Guard idempotence abonnement : un renouvellement crée toujours une
    // nouvelle subscription (status pending) ; une subscription déjà active
    // sur la même référence est donc un rejeu → on ignore.
    if (sub.status === 'active' && isSuccess) {
      logger.log('[Djomy Webhook] Subscription already active, ignoring duplicate')
      return NextResponse.json({ received: true, already_processed: true })
    }
    ...
    // Période active en cours (autre abonnement) : sert de base au calcul de
    // renouvellement ET de garde-fou pour ne pas downgrader un Pro valide
    // dont le renouvellement aurait simplement échoué.
    const activeExpiry = await fetchActiveSubscriptionExpiry(supabase, sub.user_id, sub.id)
    const hasValidActiveSub = !!activeExpiry && new Date(activeExpiry) > now

    // Nouvelle échéance : prolonge depuis la période active en cours
    // (pas de perte de jours en cas de renouvellement anticipé),
    // sinon départ à aujourd'hui.
    let expiresAt: Date | null = null
    if (isSuccess) {
      expiresAt = computeExpiryDate(activeExpiry ?? sub.expires_at, now)
    }
    ...
    if (payErr) {
      logger.error('[Djomy Webhook] payments insert error:', payErr.message)
    ...
    // En cas d'échec de paiement, on ne downgrade que si l'utilisateur n'a
    // plus aucun abonnement actif valide (renouvellement raté ≠ perte du plan).
    const finalPlan = isSuccess || hasValidActiveSub ? 'pro' : 'free'
    ...
    // ── 11b. Un seul abonnement actif : expirer les précédents (renouvellement) ──
    if (isSuccess) {
      await expireSupersededSubscriptions(supabase, sub.user_id, sub.id)
    }
    ...
    logger.error('[Djomy Webhook Error]', message)
